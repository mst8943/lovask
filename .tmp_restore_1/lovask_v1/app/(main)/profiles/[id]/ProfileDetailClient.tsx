...
export default function ProfileDetailClient() {
    ...
    const profileEducation = profile.education || profile.egitim || profile.school;
    const profileSmoking = profile.smoking || profile.sigara || profile.smoke;
    const profileAlcohol = profile.alcohol || profile.alkol || profile.drink;
    const profileReligion = profile.religion || profile.din || profile.faith;
    const profileChildren = profile.kids_status || profile.children || profile.cocuk || profile.kids;
    const profileLifestyle = profile.lifestyle;
    const profileLookingFor = profile.looking_for_genders;
    const profileRelationshipType = profile.relationship_type;
