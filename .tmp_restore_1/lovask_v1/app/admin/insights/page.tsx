        ...
        <div className="space-y-6">
            <div className="flex items-end justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-400">Aşırı aktivite / harcama görünümü</p>
                </div>
            ...
            <div className="flex flex-wrap gap-2 text-xs">
                {(['1h', '6h', '24h'] as const).map((key) => (
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-full border ${timeframe === key ? 'bg-pink-500/20 text-pink-200 border-pink-500/40' : 'bg-white/5 text-gray-300 border-white/10'}`}
                    >
                        ...
                        <div className="space-y-2 text-xs">
                            {topSpends.map((r, idx) => (
                                <div key={r.user_id} className="flex items-center justify-between">
                                    <span className="text-gray-400">#{idx + 1}</span>
                                    <span className="truncate">{r.user_id}</span>
                                    <span className="text-emerald-300">{Math.round(r.amount)}</span>
                                </div>
                            ))}
                        </div>
                    )}
