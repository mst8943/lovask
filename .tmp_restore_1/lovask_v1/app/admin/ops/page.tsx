...
export default function AdminOpsHubPage() {
    ...
    const getQueueSla = (type: CaseType) => {
        ...
        return queueConfig[type].slaHours
    }
            ...
            <div className="glass-panel p-4 rounded-2xl flex flex-wrap items-center gap-3 text-xs">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {(['all', 'mine', 'unassigned'] as const).map((key) => (
                        <button
                            ...
                            className={`px-3 py-1.5 rounded-full border ${assignFilter === key ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-600 border-slate-200'}`}
                        >
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-full border ${sortBy === 'newest' ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-600 border-slate-200'}`}
                    >
                        En yeni
                    </button>
                    <button
                        onClick={() => setSortBy('sla')}
                        className={`px-3 py-1.5 rounded-full border ${sortBy === 'sla' ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-600 border-slate-200'}`}
                    >
                        SLA yakın
                    </button>
                </div>
