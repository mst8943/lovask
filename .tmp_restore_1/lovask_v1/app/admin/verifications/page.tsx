        ...
        <div className="space-y-6">
            ...
            <div className="glass-panel p-3 rounded-2xl flex flex-wrap items-center gap-2 text-xs">
                ...
                {['all', ...Array.from(new Set(rows.map((r) => r.type)))].map((key) => (
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-full border ${typeFilter === key ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-600 border-slate-200'}`}
                    >
                        {key === 'all' ? 'Tip' : key}
                    </button>
                ))}
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs">
                ...
                <Button
                    ...
                    variant="secondary"
                    className="px-3 py-2 rounded-lg bg-emerald-500/20 text-emerald-200 text-xs disabled:opacity-40"
                ...
                <Button
                    ...
                    variant="secondary"
                    className="px-3 py-2 rounded-lg bg-red-500/20 text-red-200 text-xs disabled:opacity-40"
                    disabled={selectedIds.length === 0}
                >
                    Toplu reddet
                </Button>
                        ...
                        <div key={r.id} className="glass-panel p-4 rounded-2xl space-y-2">
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className={`text-xs px-2 py-1 rounded-full ${r.status === 'approved' ? 'bg-emerald-500/20 text-emerald-300' : r.status === 'rejected' ? 'bg-red-500/20 text-red-300' : 'bg-yellow-500/20 text-yellow-300'}`}>
                                        {r.status}
                                    </span>
                                </div>
                            </div>
                            ...
                            {r.device_info && (
                                <div className="text-xs text-gray-400">Cihaz: {r.device_info?.user_agent || '?'}</div>
                            )}
                            ...
                            <div className="flex items-center gap-2">
                                <Button
                                    ...
                                    variant="secondary"
                                    className="px-3 py-2 rounded-lg bg-emerald-500/20 text-emerald-200 text-xs"
                                >
                                    Onayla
                                </Button>
                                <Button
                                    ...
                                    variant="secondary"
                                    className="px-3 py-2 rounded-lg bg-red-500/20 text-red-200 text-xs"
                                >
                                    Reddet
                                </Button>
                            </div>
