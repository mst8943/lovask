        ...
        <div className="space-y-6">
            <div className="flex items-end justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-400">Jeton ve odeme hareketleri</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
                ...
                <div className="glass-panel p-4 rounded-2xl">
                    <div className="text-xs text-gray-400">Toplam gider</div>
                ...
                <div className="glass-panel p-4 rounded-2xl">
                    ...
                    <div className={`text-xl font-semibold ${net >= 0 ? 'text-emerald-300' : 'text-rose-300'}`}>
                ...
                <div className="glass-panel p-4 rounded-2xl">
                    <div className="text-xs text-gray-400">Max Islem</div>
                    <div className="text-xl font-semibold">
            ...
            <div className="glass-panel p-4 rounded-2xl">
                <div className="flex flex-wrap items-center gap-2 text-xs mb-3">
                    ...
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-full border ${dateFilter === 'all' ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-600 border-slate-200'}`}
                    >
                        Tum zaman
                    </button>
                    <button
                        onClick={() => setDateFilter('7d')}
                        className={`px-3 py-1.5 rounded-full border ${dateFilter === '7d' ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-600 border-slate-200'}`}
                    >
                        7 gun
                    </button>
                    <button
                        onClick={() => setDateFilter('30d')}
                        className={`px-3 py-1.5 rounded-full border ${dateFilter === '30d' ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-600 border-slate-200'}`}
                    >
                        30 gun
                    </button>
                </div>
                ...
                <div className="mt-3 flex items-center gap-2 text-xs">
                    ...
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-full border ${direction === 'all' ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-600 border-slate-200'}`}
                    >
                        Tum
                    </button>
                </div>
                {topReasons.length > 0 && (
                    <div className="mt-3 flex flex-wrap items-center gap-2 text-[10px] text-gray-400">
                        ...
                        {topReasons.map(([reason, count]) => (
                            <span key={reason} className="px-2 py-1 rounded-full border border-white/10 bg-white/10">
            ...
            <div className="glass-panel rounded-2xl overflow-hidden">
                <div className="grid grid-cols-[1fr_1.3fr_1.2fr_1.1fr_1fr_1.2fr] gap-0 text-xs uppercase tracking-wider text-gray-400 border-b border-white/10 bg-white/5">
                        ...
                        return (
                            <div key={r.id} className="grid grid-cols-[1fr_1.3fr_1.2fr_1.1fr_1fr_1.2fr] gap-0 text-sm">
                                <div className="px-4 py-3">
                                    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs bg-white/10 border border-white/10">
                                        {labelType(r.type)}
                                    </span>
                                    {r.amount < 0 && (
                                ...
                                <div className="px-4 py-3 text-xs text-gray-300 break-words">
                                    <div className="text-gray-300">{getMatchLabel(matchId)}</div>
                                    <div className="flex items-center gap-2 text-[10px] text-gray-500">
                                        {shortId(matchId)}
                                        {matchId && (
                                            <button
                                                onClick={() => navigator.clipboard.writeText(matchId)}
                                                className="px-2 py-0.5 rounded-full border border-white/10 bg-white/10 text-[10px]"
                                            >
                                                Kopyala
                                            </button>
                                        )}
                                ...
                                <div className="px-4 py-3 text-xs text-gray-300 break-words">
                                    <div className="text-gray-300">{getUserLabel(r.user_id)}</div>
                                    <div className="flex items-center gap-2 text-[10px] text-gray-500">
                                        {shortId(r.user_id)}
                                        {r.user_id && (
                                            <button
                                ...
                                <div className={`px-4 py-3 font-semibold ${r.amount >= 0 ? 'text-emerald-300' : 'text-rose-300'}`}>
                            ...
                            <div className="px-4 py-3 text-xs text-gray-400">
                                ...
                                <div className="text-[10px] text-gray-500">
                                    <button
                                        ...
                                        className="px-2 py-0.5 rounded-full border border-white/10 bg-white/10 text-[10px] mt-1"
                                    >
                                        Islem ID kopyala
                                    </button>
                                    <button
                                        ...
                                        className="px-2 py-0.5 rounded-full border border-white/10 bg-white/10 text-[10px] mt-1 ml-2"
                                    >
                                        Refund case
                                    </button>
                                    <button
            ...
            {filtered.length === 0 && (
                <div className="text-sm text-gray-400">Islem yok.</div>
            )}
        </div>
    )
}