        ...
        <div className="space-y-6">
            <div className="flex items-end justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-400">Admin audit log</p>
                </div>
            ...
            <div className="glass-panel rounded-2xl overflow-hidden">
                <div className="grid grid-cols-[1.3fr_1fr_1fr_1fr] gap-0 text-xs uppercase tracking-wider text-gray-400 border-b border-white/10 bg-white/5">
                ...
                <div className="divide-y divide-white/5">
                    {filtered.map((r) => (
                        <div key={r.id} className="grid grid-cols-[1.3fr_1fr_1fr_1fr] gap-0 text-sm">
                            <div className="px-4 py-3 text-xs text-gray-300">{r.action}</div>
                            <div className="px-4 py-3 text-xs text-gray-300">{r.target_table || '-'}</div>
                            <div className="px-4 py-3 text-xs text-gray-300">{r.target_id || '-'}</div>
                            <div className="px-4 py-3 text-xs text-gray-400">{new Date(r.created_at).toLocaleString()}</div>
                        </div>
                    ))}
                </div>
            </div>
