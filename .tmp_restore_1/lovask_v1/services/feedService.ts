...
export const fetchFeedPage = async (
    ...
    const profiles = (data || []).map((row: { users?: { is_premium?: boolean; last_active_at?: string | null } }) => ({
        ...row,
        is_premium: row.users?.is_premium || false,
        last_active_at: row.users?.last_active_at || null,
    })) as ProfileWithMeta[]
