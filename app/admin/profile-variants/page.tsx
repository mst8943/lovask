...
export default function AdminProfileVariantsPage() {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            const normalized = ((rows || []) as RawVariant[]).map((row) => ({
                ...row,
        ...
        <div className="space-y-6">
            ...
            <div className="glass-panel p-4 rounded-xl flex items-center justify-between">
                ...
                <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs border ${variantsEnabled ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-white/5 border-white/10 text-gray-300'}`}
                >
            ...
            {!hasConfig && (
                <div className="glass-panel p-4 rounded-xl text-xs text-amber-800 border border-amber-200 bg-amber-50">
            ...
            <div className="flex items-center gap-2 text-xs">
                {(['pending', 'approved', 'all'] as const).map((key) => (
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-full border ${filter === key ? 'bg-rose-50 text-rose-800 border-rose-200' : 'bg-white/5 text-gray-300 border-white/10'}`}
                    >
                        ...
                        {filtered.map((row) => {
                            ...
                            return (
                                <div key={row.id} className="grid grid-cols-[1.2fr_1fr_0.8fr_0.6fr] gap-0 text-sm">
                                    ...
                                    <div className="px-4 py-3">
                                        <span className={`text-[10px] px-2 py-1 rounded-full border ${approved ? 'bg-emerald-50 text-emerald-800 border-emerald-200' : 'bg-amber-50 text-amber-800 border-amber-200'}`}>
                                    ...
                                    <div className="px-4 py-3 text-right">
                                        <button
                                            ...
                                            className={`px-3 py-1.5 rounded-full border text-xs ${approved ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-rose-50 border-rose-200 text-rose-800'}`}
                                        >
