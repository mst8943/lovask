        ...
        <div className="space-y-6">
            <div className="flex items-end justify-between">
                <div>
                    ...
                    <p className="text-sm text-slate-500">Son 7 gunluk ozet</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="glass-panel rounded-2xl overflow-hidden">
                    <div className="grid grid-cols-[1.4fr_0.6fr] gap-0 text-xs uppercase tracking-wider text-slate-500 border-b border-slate-200 bg-white">
                        ...
                        {topBots.map((row) => {
                            ...
                            return (
                                <div key={row.bot_id} className="grid grid-cols-[1.4fr_0.6fr] gap-0 text-sm">
                                    <div className="px-4 py-3 text-xs text-slate-600">{bot?.display_name || row.bot_id}</div>
                                    <div className="px-4 py-3 text-xs text-slate-600">{row.replies}</div>
                                </div>
                            )
                        })}
                        {topBots.length === 0 && (
                            <div className="px-4 py-6 text-xs text-slate-500">Veri yok</div>
                        )}
                    </div>
                </div>
                <div className="glass-panel rounded-2xl overflow-hidden">
                    ...
                    <div className="divide-y divide-white/5">
                        {Object.entries(safetyByCategory).map(([key, value]) => (
                            <div key={key} className="grid grid-cols-[1.4fr_0.6fr] gap-0 text-sm">
                                <div className="px-4 py-3 text-xs text-slate-600">{key}</div>
                                <div className="px-4 py-3 text-xs text-slate-600">{value}</div>
                            </div>
                        ))}
